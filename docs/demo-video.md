...
0. **Плагін `screencast-demo-maker`** — сам механізм зйомки: три скіли й агент-перевіряльник.
   ...
   У цьому репозиторії ставити нічого не треба: `.claude/settings.json` уже підключає
   плагін, і Claude Code вмикає його, щойно ти довіришся теці проєкту. В іншому проєкті —
   дві команди:
   ...
   Оновити до нової версії (потім перезапусти Claude Code):

   ```sh
   claude plugin marketplace update screencast-demo-maker
   ```

   Що з решти пунктів уже є, а чого бракує, покаже одна перевірка: скажи Claude
   «перевір, чи все готово до зйомки». Він запустить `doctor.mjs` з плагіна, доставить
   Playwright і для кожної проблеми дасть команду, що її виправляє.

1. **Свій акаунт ElevenLabs** — сервіс, що перетворює текст на голос. У кожного, хто
   знімає, власний акаунт і ключ: чужим ключем не користуйся і свій не передавай. Скіли
   спершу перевіряють ключ і, якщо його немає, пояснюють, як отримати. Безкоштовного
   тарифу вистачає: 10 тисяч символів на місяць, п'ятихвилинне відео з'їдає близько
   3500. Без ключа теж можна: безкоштовний голос macOS (`"provider": "say"` у
   `config.json`) — роботизований, але для чернетки згодиться.

   Створи ключ на https://elevenlabs.io/app/settings/api-keys, скопіюй його і виконай
   команду нижче. Ключ не з'явиться на екрані й не ляже у файл — він
...
3. **Другий монітор.** Зйомка йде на ньому, а ти тим часом працюєш на вбудованому
   екрані й бачиш, що відбувається. З одним екраном теж знімає, але поки йде зйомка,
   ноутбуком користуватись не вийде.
...
| Частина | Де | Навіщо |
|---|---|---|
| скіл `demo-setup` | плагін `screencast-demo-maker` | налаштовує новий проєкт: питає, що знімати, і пише `config.json` |
...
| перевірка середовища | `doctor.mjs` у плагіні | каже, чого бракує, і як це виправити |

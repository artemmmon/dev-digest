 ...
 * `agent_runs` + `run_traces` (one trace doc per run). `pr_intent` (L03)
 * belongs to `modules/intent` now — reviews reaches it through the structural
 * `ReviewDeps.intent` port, wired in the container. Workspace scoping is

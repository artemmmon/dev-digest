...
d('Settings: feature models + secrets status (Testcontainers pg)', () => {
  ...
  it('resolveFeatureModel: registry default until overridden, then the workspace choice', async () => {
    ...
    expect(await getFeatureModelOverride(app.container.settingsRepo, workspaceId, 'onboarding')).toBeUndefined();
    expect(await resolveFeatureModel(app.container.settingsRepo, workspaceId, 'onboarding')).toEqual({
      provider: 'openrouter',
    ...
    expect(await resolveFeatureModel(app.container.settingsRepo, workspaceId, 'risk_brief')).toEqual({
      provider: 'openai',
      model: 'gpt-4.1',
    });

...
feeding it — `skills` (L02, fed by the server: one `### Skill: <name>` block per enabled skill), `memory` (L07), `specs` (L05), `callers` — plus a
`reduce()`/map-reduce path and a `toReview()` CI payload helper used from L06.
The server passes the diff, system prompt, repo map and, since L02, the agent's skills; the
other slots are omitted, so `assemblePrompt` simply leaves those sections out.
